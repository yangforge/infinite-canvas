...
+ [新增] 渠道协议新增火山方舟 Seedream，内置文生图、改图请求和模型列表拉取；Grok 生图沿用 OpenAI 协议即可（端点一致）。
+ [新增] 渠道新增「改图用 JSON 请求体」开关，开启后改图以 application/json 提交，适配 xAI 与只接受 JSON 的中转站。
+ [新增] 视频渠道新增 AutoDL ComfyUI 协议：模型名填工作流 ID，提交任务后按任务 ID 轮询结果，每个模型可自定义 JSON 请求参数模板。
+ [新增] 配置弹窗新增「从链接导入」：粘贴配置文件的网络地址（http(s)://）即可拉取并导入，与本地文件导入等效。
+ [修复] 画布视频节点手动「获取任务状态」对 AutoDL 渠道误用 OpenAI 轮询路径 `/videos/{id}`；现按真实 provider 路由到 `/comfyui_workflow/result/{id}`。
+ [新增] 配置弹窗默认拉取站点根目录 `/config.json` 作为默认配置：每次刷新自动获取并与本地配置合并，其中的渠道在「渠道」列表带「默认」标记，但均可编辑、删除（编辑默认渠道会转为用户自有渠道并持久化）；用户仍可自行新增渠道；文件缺失或格式错误时静默忽略；导出配置包含所有渠道（含站点默认渠道）。
